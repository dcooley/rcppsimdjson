#ifndef R_RCPPSIMDJSON_FROM_JSON_SIMPLIFY_H
#define R_RCPPSIMDJSON_FROM_JSON_SIMPLIFY_H
...
namespace from_json {
  ...
  inline SEXP array_to_vector(
    const simdjson::dom::array& array,
    bool& simplify
  ) {
    int r_type = 0;
    ...
    for( i = 0; i < arr_len; ++i ) {
      ...
      case simdjson::dom::element_type::INT64: {
        int64_t x = array.at(i).get< int64_t >();
        out[i] = x;
      ...
      //   uint64_t x = array.at(i).get< uint64_t >();
      //   out[i] = x;
      //   r_type = INTSXP > r_type ? INTSXP : r_type;
      //   break;
      // }
      ...
      case simdjson::dom::element_type::DOUBLE: {
        double x = array.at(i).get< double >();
        out[i] = x;
      ...
      case simdjson::dom::element_type::STRING: {
        std::string_view x = array.at(i).get< std::string_view >();
        std::string s = std::string( x ).c_str();
        out[i] = s;
      ...
      default: {
        Rcpp::stop("RcppSimdJson - array_to_vector unsupported type");
      }
    ...
    if( simplify ) {
      return simplify_vector( out, r_type, 1 );
    }
