...
//`int64_t` can be downcasted when between `INT_MAX` and 1+`INT_MIN`/`NA_INTEGER`
template <>
...
// - otherwise `x` is coerced to provided `Int64_R_Type`
...
// - otherwise `x` is coerced to an `Rcpp::Vector` following provided `Int64_R_Type`
template <Int64_R_Type int64_opt>
